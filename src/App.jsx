...
const ProtectedRoute = ({ allowedRoles }) => {
  ...
  try {
    ...
    if (!isAllowed) {
      ...
      // ĐÃ SỬA: So sánh với chuỗi viết thường (lowercase)
      if (userRole === "admin") return <Navigate to="/admin" replace />;
      if (userRole === "instructor")
        return <Navigate to="/introductor" replace />;
      if (userRole === "qa" || userRole === "qualityassurance")
        return <Navigate to="/qa" replace />;
      if (userRole === "academic" || userRole === "academicstaff")
        return <Navigate to="/academic" replace />;
      if (userRole === "trainingmanager")
        return <Navigate to="/trainingmanager" replace />;
      ...
      <Routes>
        ...
        <Route element={<ProtectedRoute allowedRoles={["Instructor"]} />}>
          <Route path="/introductor" element={<IntroductorLayout />}>
            <Route
              index
              element={<Navigate to="/introductor/classes" replace />}
            />
        ...
        {/* Protected QA Routes */}
        <Route
          element={<ProtectedRoute allowedRoles={["QA", "QualityAssurance"]} />}
        >
