...
const EtrManagement = () => {
    ...
    const loadData = async () => {
      try {
        ...
        const detailsArr = await Promise.all(
          etrsArr.map((e) =>
            api
              .get(`/Etr/${e.etrCourseRecordId || e.eTRCourseRecordId}`)
              .catch(() => null),
          ),
        );
        const srMap = {};
  ...
  const mergeEtrData = (
    ...
    return etrs.map((etr) => {
      ...
      const profile = account
        ? profiles.find((p) => p.accountId === account.accountId)
        : null;
  ...
  const handleUploadEvidence = async (e) => {
    ...
    try {
      ...
      const etrDetail = await api
        .get(`/Etr/${selectedRecord.etrId}`)
        .catch(() => null);
  ...
  const imgCount = selectedRecord
    ? selectedRecord.evidenceList.filter(
        (f) => f.type === "PHOTO" || f.type === "IMAGE",
      ).length
    : 0;
  const pdfCount = selectedRecord
    ? selectedRecord.evidenceList.filter(
        (f) => f.type === "PDF DOC" || f.type === "PDF",
      ).length
    : 0;
  const sigCount = selectedRecord
    ? selectedRecord.evidenceList.filter((f) => f.type === "SIGNATURE").length
    : 0;

  const filteredFiles = selectedRecord
    ? selectedRecord.evidenceList.filter((file) => {
        const matchesSearch =
          file.name.toLowerCase().includes(fileSearchQuery.toLowerCase()) ||
          file.tag.toLowerCase().includes(fileSearchQuery.toLowerCase());

        if (fileCategoryFilter === "ALL") return matchesSearch;
        if (fileCategoryFilter === "IMAGE")
          return (
            matchesSearch && (file.type === "PHOTO" || file.type === "IMAGE")
          );
        if (fileCategoryFilter === "PDF")
          return (
            matchesSearch && (file.type === "PDF DOC" || file.type === "PDF")
          );
        if (fileCategoryFilter === "SIGNATURE")
          return matchesSearch && file.type === "SIGNATURE";
  ...
  if (viewMode === "evidence" && selectedRecord) {
    return (
      <>
        <div
          className="evidence-management-page flex flex-col justify-start items-start w-full relative bg-[#f5f7fa]"
          style={{
            display: "flex",
        ...
        >
          <div
            className="flex flex-col-reverse justify-start items-start self-stretch flex-grow overflow-hidden"
            style={{ width: "100%" }}
          >
            {/* Content Pane */}
            <div
              className="flex flex-col justify-start items-start self-stretch flex-grow overflow-hidden gap-6 p-8"
              style={{ width: "100%" }}
            >
              {/* Header section (breadcrumbs and title) */}
              <div
                className="flex justify-between items-end self-stretch flex-grow-0 flex-shrink-0 pb-4"
                style={{ width: "100%" }}
              >
                <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-4">
                  {/* Breadcrumbs */}
                  <div
                    className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 gap-2 px-4 py-2 rounded-lg bg-[#002147]"
                    style={{ display: "inline-flex", alignSelf: "flex-start" }}
                  >
                    <p
                      className="text-[10px] font-bold text-left uppercase text-white"
                      style={{ margin: 0 }}
                    >
                      {tr("ACADEMIC PORTAL")}
                    </p>
                    ...
                    <p
                      className="text-[10px] font-bold text-left uppercase text-[#c5a059]"
                      style={{ margin: 0 }}
                    >
                  ...
                  <div className="flex items-baseline gap-2">
                    <span
                      className="text-3xl italic text-left text-[#c5a059] font-light"
                      style={{ fontSize: "30px" }}
                    >
                      {tr("Evidence Management")}
                    </span>
                  ...
                  <div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 gap-6">
                    <div className="flex justify-start items-center gap-2 px-4 py-2 rounded-lg bg-[#002147] border border-[#c5a059]/40">
                      <p
                        className="text-[10px] font-bold text-left uppercase text-white"
                        style={{ margin: 0 }}
                      >
              ...
              >
                ...
                <div className="flex justify-start items-center gap-2">
                  ...
                  >
                    <p
                      className={`text-xs font-bold text-center uppercase ${fileCategoryFilter === "ALL" ? "text-[#002147]" : "text-[#002147]/60"}`}
                      style={{ margin: 0 }}
                    >
                  ...
                  >
                    <p
                      className={`text-xs font-bold text-center uppercase ${fileCategoryFilter === "IMAGE" ? "text-[#002147]" : "text-[#002147]/60"}`}
                      style={{ margin: 0 }}
                    >
                  ...
                  >
                    <p
                      className={`text-xs font-bold text-center uppercase ${fileCategoryFilter === "PDF" ? "text-[#002147]" : "text-[#002147]/60"}`}
                      style={{ margin: 0 }}
                    >
                  ...
                  >
                    <p
                      className={`text-xs font-bold text-center uppercase ${fileCategoryFilter === "SIGNATURE" ? "text-[#002147]" : "text-[#002147]/60"}`}
                      style={{ margin: 0 }}
                    >
              ...
              {evidenceAccessDenied && (
                ...
                >
                  ...
                  <span>
                    {tr("Tài khoản của bạn chưa được backend cho phép")}{" "}
                    <strong>{tr("đọc")}</strong>{" "}
                    {tr(
              ...
              >
                ...
                >
                  {/* Table Header */}
                  <div className="table-header evidence-table-grid">
                          ...
                          >
                            ...
                            <div className="flex justify-start items-center">
                              <input
                                ...
                                onChange={(e) =>
                                  handleToggleFileSelect(file.name, e)
                                }
                              />
                            </div>
                            ...
                            <div className="flex flex-col gap-1 pr-4">
                              <span
                                className="text-sm font-bold text-[#002147] truncate"
                                title={file.name}
                              >
                                {file.name}
                              </span>
                            ...
                            <div className="flex items-center justify-center">
                              {file.status === "Verified" ? (
                                ...
                                <div className="flex justify-start items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-50 border border-amber-200">
                                  ...
                                  <span className="text-[11px] font-bold text-[#B45309] uppercase">
                                    {tr("Pending QA")}
                                  </span>
                                </div>
                              ) : (
                                <div className="relative-tooltip-container flex items-center justify-center">
                                  <div className="flex justify-start items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-50 border border-red-200">
                                    ...
                                    >
                                      <path
                                        d="M5 7.5C5.14167 7.5 5.26042 7.45208 5.35625 7.35625C5.45208 7.26042 5.5 7.14167 5.5 7C5.5 6.85833 5.45208 6.73958 5.35625 6.64375C5.26042 6.54792 5.14167 6.5 5 6.5C4.85833 6.5 4.73958 6.54792 4.64375 6.64375C4.54792 6.73958 4.5 6.85833 4.5 7C4.5 7.14167 4.54792 7.26042 4.64375 7.35625C4.73958 7.45208 4.85833 7.5 5 7.5ZM4.5 5.5H5.5V2.5H4.5V5.5ZM5 10C4.30833 10 3.65833 9.86875 3.05 9.60625C2.44167 9.34375 1.9125 8.9875 1.4625 8.5375C1.0125 8.0875 0.65625 7.55833 0.39375 6.95C0.13125 6.34167 0 5.69167 0 5C0 4.30833 0.13125 3.65833 0.39375 3.05C0.65625 2.44167 1.0125 1.9125 1.4625 1.4625C1.9125 1.0125 2.44167 0.65625 3.05 0.39375C3.65833 0.13125 4.30833 0 5 0C5.69167 0 6.34167 0.13125 6.95 0.39375C7.55833 0.65625 8.0875 1.0125 8.5375 1.4625C8.9875 1.9125 9.34375 2.44167 9.60625 3.05C9.86875 3.65833 10 4.30833 10 5C10 5.69167 9.86875 6.34167 9.60625 6.95C9.34375 7.55833 8.9875 8.0875 8.5375 8.5375C8.0875 8.9875 7.55833 9.34375 6.95 9.60625C6.34167 9.86875 5.69167 10 5 10ZM5 9C6.11667 9 7.0625 8.6125 7.8375 7.8375C8.6125 7.0625 9 6.11667 9 5C9 3.88333 8.6125 2.9375 7.8375 2.1625C7.0625 1.3875 6.11667 1 5 1C3.88333 1 2.9375 1.3875 2.1625 2.1625C1.3875 2.9375 1 3.88333 1 5C1 6.11667 1.3875 7.0625 2.1625 7.8375C2.9375 8.6125 3.88333 9 5 9Z"
                                        fill="#B91C1C"
                                      />
                                    </svg>
                                    <span className="text-[11px] font-bold text-[#B91C1C] uppercase">
                                      {tr("Rejected")}
                                    </span>
                                  </div>
                                  <div className="tooltip-card flex flex-col justify-start items-center p-3 rounded-lg bg-[#002147] border-b-4 border-[#c5a059] shadow-lg w-56">
                                    <p
                                      className="text-[11px] text-center text-white leading-normal"
                                      style={{ margin: 0 }}
                                    >
                                      {tr("Lý do: ")}
                                      {file.rejectReason ||
                                        tr("Tập tin không đạt yêu cầu.")}
                                    </p>
                                    <div
                                      style={{
                                        width: 0,
            ...
            >
              <div className="flex justify-start items-center gap-3">
                <div
                  className="flex justify-center items-center w-10 h-10 rounded-xl bg-gradient-to-br from-[#c5a059] to-[#d4af37]"
                ...
                <p
                  className="text-sm font-bold text-left uppercase text-[#002147] tracking-wider"
                  style={{ margin: 0 }}
                >
          ...
          createPortal(
            <div className="modal-overlay">
              <div
                ...
                style={{ width: "860px", maxWidth: "96%" }}
              >
                <header className="modal-header">
                  <h2>{tr("XEM TRƯỚC TÀI LIỆU MINH CHỨNG")}</h2>
                  <button
                    className="close-btn"
                    type="button"
                ...
                >
                  {/* Chi tiết tổng quan minh chứng (metadata) */}
                  <div
                    className="flex flex-col rounded-xl bg-white border border-slate-200 shadow-sm mb-4"
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      width: "100%",
                  ...
                  >
                    <div
                      className="px-4 py-2.5 rounded-t-xl bg-[#002147] flex justify-between items-center"
                      style={{
                        display: "flex",
                    ...
                    >
                      <span
                        ...
                        style={{ color: "#c5a059", letterSpacing: "0.05em" }}
                      >
                        {tr("Thông tin chi tiết minh chứng")}
                      </span>
                      <span
                    ...
                    >
                      <div className="flex justify-between items-center gap-3">
                        <span
                          className="text-[11px] font-bold uppercase text-[#64748b]"
                          style={{ margin: 0 }}
                        >
                          {tr("MÃ ETR HỒ SƠ")}
                        </span>
                      ...
                      <div className="flex justify-between items-center gap-3">
                        ...
                        <span
                          className="text-xs font-bold text-[#002147] truncate"
                  ...
                  >
                    <div
                      className={`flex justify-center items-center w-12 h-12 rounded-xl flex-shrink-0 ${
                        previewFile.type === "PDF DOC" ||
                    ...
                    <div
                      style={{ flexGrow: 1, minWidth: 0, paddingLeft: "16px" }}
                    >
                      <h3
                        className="text-base font-bold text-[#002147] truncate"
                        style={{ margin: 0 }}
                        title={previewFile.name}
                      >
                        {previewFile.name}
                    ...
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      <span
                        className={`inline-block px-3 py-1 rounded text-xs font-black ${
                          previewFile.status === "Verified"
                            ? "bg-green-100 text-green-800"
                            : previewFile.status === "Pending QA"
                              ? "bg-amber-100 text-amber-800"
                              : "bg-red-100 text-red-800"
                        }`}
                      >
                  ...
                  <div
                    className="flex flex-col items-center justify-center bg-white rounded-xl border border-slate-200 shadow-inner p-6 min-h-[300px] relative overflow-hidden"
                    style={{
                      display: "flex",
                      flexDirection: "column",
                      alignItems: "center",
                  ...
                  >
                    ...
                    {(previewFile.type === "PDF DOC" ||
                      previewFile.type === "PDF") && (
                      <div
                        className="w-full max-w-[400px] border border-slate-200 rounded-lg shadow bg-slate-50 flex flex-col"
                        style={{
                          minHeight: "260px",
                          display: "flex",
                          flexDirection: "column",
                      ...
                      >
                        <div
                          className="h-10 bg-slate-100 border-b border-slate-200 flex items-center justify-between px-4 rounded-t-lg"
                          style={{
                            display: "flex",
                            justifyContent: "space-between",
                            alignItems: "center",
                        ...
                        <div
                          className="p-6 flex-grow flex flex-col gap-4 bg-white"
                          style={{
                            display: "flex",
                            flexDirection: "column",
                            gap: "16px",
                        ...
                        >
                          ...
                          >
                            ...
                            <div>
                              <div className="text-xs font-bold text-slate-800">
                                {tr("ETR TRAINING SYSTEM")}
                              </div>
                              <div
                                className="text-[10px] font-bold text-slate-400"
                                style={{ margin: 0 }}
                              >
                                {tr("OFFICIAL TRAINING RECORD")}
                              </div>
                            </div>
                          </div>
                          <div
                          ...
                          >
                            <div
                              className="h-3 w-3/4 bg-slate-100 rounded"
                              style={{
                                height: "12px",
                                width: "75%",
                            ...
                            />
                            <div
                              className="h-3 w-5/6 bg-slate-100 rounded"
                              style={{
                                height: "12px",
                                width: "83.33%",
                                backgroundColor: "#f1f5f9",
                            ...
                            />
                            <div
                              className="h-3 w-2/3 bg-slate-100 rounded"
                              style={{
                                height: "12px",
                                width: "66.67%",
                                backgroundColor: "#f1f5f9",
                            ...
                            />
                            <div
                              className="h-3 w-1/2 bg-slate-100 rounded"
                              style={{
                                height: "12px",
                                width: "50%",
                                backgroundColor: "#f1f5f9",
                          ...
                          <div
                            className="mt-auto border-t border-slate-100 pt-3 flex justify-between items-center text-[10px] font-bold text-slate-400"
                            style={{
                              marginTop: "auto",
                    ...
                    {(previewFile.type === "PHOTO" ||
                      previewFile.type === "IMAGE") && (
                      <div
                        className="w-full max-w-[450px] border border-slate-200 rounded-lg shadow bg-slate-900 flex flex-col items-center justify-center p-4 relative"
                        style={{
                          minHeight: "260px",
                          display: "flex",
                          flexDirection: "column",
                      ...
                      >
                        ...
                        <svg
                          ...
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                          className="relative z-10 text-[#c5a059]"
                          style={{ position: "relative", zIndex: 10 }}
                        >
                          <path
                            d="M20 60 L100 20 L180 50 L100 110 Z"
                            stroke="currentColor"
                            strokeWidth="1.5"
                            strokeDasharray="3 3"
                          ...
                          />
                          <path
                            d="M20 75 L20 85 M180 65 L180 75"
                            stroke="currentColor"
                            strokeWidth="0.8"
                          />
                          <path
                        ...
                        <div
                          className="absolute bottom-2 left-2 right-2 flex justify-between text-[9px] font-bold text-slate-500 z-10"
                          style={{
                            display: "flex",
                            justifyContent: "space-between",
                            width: "100%",
                    ...
                    {previewFile.type === "SIGNATURE" && (
                      <div
                        className="w-full max-w-[400px] border-2 border-dashed border-[#c5a059]/40 rounded-xl bg-[#002147]/5 p-6 flex flex-col items-center justify-center gap-4 text-center"
                        style={{
                          minHeight: "260px",
                          display: "flex",
                          flexDirection: "column",
                      ...
                      >
                        <div
                          className="w-14 h-14 rounded-full bg-[#002147]/10 flex items-center justify-center text-[#002147]"
                          style={{
                            width: "56px",
                            height: "56px",
                            borderRadius: "50%",
                        ...
                        >
                          ...
                          >
                            <path
                              fillRule="evenodd"
                              d="M2.166 4.9C2.044 5.02 2 5.19 2 5.37v9.26c0 .18.044.35.166.47.66.66 2.052 1.9 4.334 1.9 2.282 0 3.674-1.24 4.334-1.9.122-.12.166-.29.166-.47V5.37c0-.18-.044-.35-.166-.47C10.174 4.24 8.782 3 6.5 3c-2.282 0-3.674 1.24-4.334 1.9zm13.168.03c-.2-.06-.412-.03-.59.09-.176.12-.278.33-.278.55v9.12c0 .24.12.46.326.58.156.09.336.12.504.07 1.488-.41 2.376-.14 3.012.38.12.1.28.15.44.15.38 0 .7-.32.7-.7V6.04c0-.28-.16-.54-.42-.64-1.04-.4-2.58-.62-3.804-.47z"
                              clipRule="evenodd"
                            />
                          </svg>
                        </div>
                        <div>
                          <div
                            className="text-xs font-black text-[#002147] tracking-wider"
                            style={{ fontWeight: "bold" }}
                          >
                            {tr("CERTIFIED DIGITAL ID SIGNATURE")}
                          </div>
                          <div
                            className="text-[10px] font-bold text-[#c5a059] uppercase mt-1"
                            style={{ color: "#c5a059" }}
                          >
                            {tr("ETR CERTIFICATION TRUST")}
                          </div>
                        </div>

                        <div
                          className="bg-white p-3 rounded-lg border border-slate-200 text-left font-mono text-[9px] w-full text-slate-500 flex flex-col gap-1 shadow-sm"
                          style={{
                            fontFamily: "monospace",
                            fontSize: "9px",
                            display: "flex",
                        ...
                        >
                          {previewFile.status === "Verified" ? (
                            <div
                              className="flex items-center gap-1 text-green-700 font-bold text-xs bg-green-50 border border-green-200 px-3 py-1 rounded-full"
                              style={{
                                color: "#15803d",
                            ...
                            previewFile.status === "Pending QA" ? (
                            <div
                              className="flex items-center gap-1 text-amber-700 font-bold text-xs bg-amber-50 border border-amber-200 px-3 py-1 rounded-full"
                              style={{
                                color: "#b45309",
                            ...
                            <div
                              className="flex items-center gap-1 text-red-700 font-bold text-xs bg-red-50 border border-red-200 px-3 py-1 rounded-full"
                              style={{
                                color: "#b91c1c",
  ...
  return (
    <div className="etr-management-page etr-qa-shell flex flex-col justify-start items-start w-full relative bg-[#f4f7fa]">
      <div
        className="etr-qa-content flex flex-col justify-start items-start self-stretch flex-grow-0 flex-shrink-0 relative gap-6 p-8"
        style={{ width: "100%" }}
      >
        {/* Progress Steps Card */}
        {selectedRecord && (
          ...
          >
            ...
            >
              <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 gap-1">
                <div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 relative gap-3">
                  ...
                  >
                    <path
                      d="M16.25 22.5V18.75H11.25V6.25H8.75V10H0V0H8.75V3.75H16.25V0H25V10H16.25V6.25H13.75V16.25H16.25V12.5H25V22.5H16.25ZM2.5 2.5V7.5V2.5ZM18.75 15V20V15ZM18.75 2.5V7.5V2.5ZM18.75 7.5H22.5V2.5H18.75V7.5ZM18.75 20H22.5V15H18.75V20ZM2.5 7.5H6.25V2.5H2.5V7.5Z"
                      fill="#C5A059"
                    />
                  </svg>
                  <p
                    className="text-xl font-black text-left uppercase text-[#002147]"
                    style={{ margin: 0 }}
                  >
                    TIẾN ĐỘ HỒ SƠ ETR
                  </p>
                ...
                <p
                  className="text-xs font-medium text-left text-slate-400"
                  style={{ margin: 0 }}
                >
                  {tr(
              ...
              <div className="flex flex-col justify-start items-start relative px-6 py-2 rounded-lg bg-[#002147] border border-[#c5a059]/30">
                <p
                  className="text-xs font-bold text-left text-[#c5a059]"
                  style={{ margin: 0 }}
                >
            ...
            <div
              className="etr-workflow-container"
              style={{
                position: "relative",
                width: "100%",
                maxWidth: "900px",
            ...
            >
              <div
                className="etr-workflow-line-bg"
                style={{
                  position: "absolute",
                  top: "24px",
                  left: "48px",
              ...
              >
                ...
                >
                  <div
                    className={`step-circle ${selectedRecord.steps.attendance ? "completed" : "pending"}`}
                    style={{
                      width: "48px",
                      height: "48px",
                      borderRadius: "12px",
                      border: `2px solid ${selectedRecord.steps.attendance ? "#c5a059" : "#e2e8f0"}`,
                      backgroundColor: selectedRecord.steps.attendance
                        ? "#002147"
                        : "#ffffff",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      color: selectedRecord.steps.attendance
                        ? "#c5a059"
                        : "#cbd5e1",
                    }}
                  >
                ...
                >
                  <div
                    className={`step-circle ${selectedRecord.steps.results ? "completed" : "pending"}`}
                    style={{
                      width: "48px",
                      height: "48px",
                      borderRadius: "12px",
                      border: `2px solid ${selectedRecord.steps.results ? "#c5a059" : "#e2e8f0"}`,
                      backgroundColor: selectedRecord.steps.results
                        ? "#002147"
                        : "#ffffff",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      color: selectedRecord.steps.results
                        ? "#c5a059"
                        : "#cbd5e1",
                    }}
                  >
                ...
                >
                  <div
                    className={`step-circle ${selectedRecord.steps.evidence ? "completed" : "pending"}`}
                    style={{
                      width: "48px",
                      height: "48px",
                      borderRadius: "12px",
                      border: `2px solid ${selectedRecord.steps.evidence ? "#c5a059" : "#e2e8f0"}`,
                      backgroundColor: selectedRecord.steps.evidence
                        ? "#002147"
                        : "#ffffff",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      color: selectedRecord.steps.evidence
                        ? "#c5a059"
                        : "#cbd5e1",
                    }}
                  >
        ...
        >
          <div
            className="flex justify-between items-center self-stretch flex-grow-0 flex-shrink-0 p-6 bg-slate-50/50 border-t-0 border-r-0 border-b border-l-0 border-slate-200"
            style={{ width: "100%" }}
          >
            <div className="flex justify-start items-center flex-grow-0 flex-shrink-0 gap-4">
              <p
                className="text-lg font-bold text-left uppercase text-[#002147]"
                style={{ margin: 0 }}
              >
              ...
              <div className="flex flex-col justify-start items-start px-3 py-1 rounded-md bg-[#002147]">
                <p
                  className="text-[10px] font-black text-left uppercase text-[#c5a059]"
                  style={{ margin: 0 }}
                >
            ...
            <div className="flex justify-start items-start gap-3">
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(166.97deg, #c5a059 -63.79%, #d4af37 163.79%)",
                }}
          ...
          <div className="table-responsive-scroll" style={{ width: "100%" }}>
            <div
              className="table-header etr-table-grid"
              ...
              {pageItems.map((record) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      cursor: "pointer",
                      borderLeft: isSelected ? "4px solid #c5a059" : "none",
                    }}
                  >
                    <div
                      className="col-code"
                      style={{ fontWeight: "700", color: "#002147" }}
                    >
                      {record.id}
                    </div>
                    ...
                    <div className="col-actions !flex !min-w-0 !flex-row !flex-wrap !items-center !justify-start !gap-2 !pr-0 xl:!justify-end">
                      ...
                      >
                        ...
                        >
                          <path
                            d="M1.33333 12C0.966667 12 0.652778 11.8694 0.391667 11.6083C0.130556 11.3472 0 11.0333 0 10.6667V1.33333C0 0.966667 0.130556 0.652778 0.391667 0.391667C0.652778 0.130556 0.966667 0 1.33333 0H12C12.3667 0 12.6806 0.130556 12.9417 0.391667C13.2028 0.652778 13.3333 0.966667 13.3333 1.33333V10.6667C13.3333 11.0333 13.2028 11.3472 12.9417 11.6083C12.6806 11.8694 12.3667 12 12 12H1.33333ZM1.33333 10.6667H12V1.33333H1.33333V10.6667Z"
                            fill="currentColor"
                          />
                        </svg>
                        <span className="sr-only xl:not-sr-only xl:whitespace-nowrap xl:text-[10px] xl:font-semibold xl:leading-none">
                          {tr("MINH CHỨNG")}
                        </span>
                      </button>
                      <button
        ...
        >
          <div
            className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 gap-3 p-5 bg-slate-50/50 border-t-0 border-r-0 border-b border-l-0 border-slate-200"
            style={{ width: "100%" }}
          >
            <svg
              width={18}
          ...
          <div className="table-responsive-scroll" style={{ width: "100%" }}>
            {/* Audit header */}
            <div
              className="table-header audit-table-grid"
              style={{
                minWidth: "800px",
                backgroundColor: "#002147",
                color: "#ffffff",
            ...
            >
              ...
              <div>{tr("HÀNH ĐỘNG")}</div>
              <div>{tr("NỘI DUNG THAY ĐỔI")}</div>
            </div>
              ...
              <div className="modal-container" style={{ width: "550px" }}>
                <header className="modal-header">
                  ...
                  <button
                    className="close-btn"
                    type="button"
                    onClick={() => setIsEvidenceUploadOpen(false)}
                    aria-label={tr("Đóng")}
                  >
                    &times;
                  </button>
                    ...
                    <div className="form-row" style={{ marginTop: "16px" }}>
                      <div className="form-group">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setUploadEvidenceTypeId(e.target.value)
                          }
                          required
                        >
                          {uploadEvidenceTypes.length === 0 ? (
                            <option value="">
                              {tr("Đang tải danh mục...")}
                            </option>
                          ) : (
                            uploadEvidenceTypes.map((et) => (
                              <option
                                key={et.evidenceTypeId}
                                value={et.evidenceTypeId}
                              >
          ...
          createPortal(
            <div className="modal-overlay">
              ...
              >
                <header className="modal-header">
                  <h2>
                    {tr("HỒ SƠ ĐÀO TẠO ĐIỆN TỬ HOÀN CHỈNH (ETR FINAL SHEET)")}
                  </h2>
                  <button
                    className="close-btn"
                    type="button"
