...
const CourseClassManagement = () => {
        ...
        setInstructorsList(
          accountsArr
            .filter(
              (a) =>
                a.roleId === 2 ||
                a.roleName === "Instructor" ||
                a.role === "Instructor",
            )
            .map((a) => {
              const profile = profilesArr.find(
                (p) => String(p.accountId) === String(a.accountId),
              );
              return {
    ...
    return assigned
      .map((a) => {
        ...
        if (a.instructorAccountId) {
          const acc = accountsArr.find(
            (x) => String(x.accountId) === String(a.instructorAccountId),
          );
          const prof = profilesArr.find(
            (p) => String(p.accountId) === String(a.instructorAccountId),
          );
          insName =
            prof?.fullName ||
            acc?.fullName ||
            acc?.username ||
            `GV #${a.instructorAccountId}`;
        }
        const sub = subjectsList.find(
          (s) => String(s.subjectId) === String(a.subjectId),
        );
    ...
    return coursesArr.map((course) => {
      ...
      return {
        ...
        classes: courseClasses.map((cls) => {
          const insName = resolveClassInstructors(
            cls,
            accountsArr,
            profilesArr,
            subjectsList,
          ...
          return {
            ...
            status:
              cls.status === "Active" || cls.status === "InProgress"
                ? "Đang diễn ra"
                : cls.status === "Upcoming" || cls.status === "Planned"
                  ? "Sắp diễn ra"
  ...
  const handleSaveCourse = async (newCourse) => {
    try {
      const subjectsPayload =
        Array.isArray(newCourse.subjects) && newCourse.subjects.length > 0
          ? newCourse.subjects
  ...
  const handleSaveUpdateCourse = async (courseId, updateData) => {
    try {
      const subjectsPayload =
        Array.isArray(updateData.subjects) && updateData.subjects.length > 0
          ? updateData.subjects
      ...
      await api.put(`/Courses/${courseId}`, {
        ...
        durationHours:
          parseInt(updateData.durationHours ?? updateData.duration) || 0,
  ...
  const handleSaveClass = async (parentCourseId, newClass) => {
    ...
    try {
      ...
      if (
        allCoursesRaw.length > 0 &&
        !allCoursesRaw.some((c) => Number(c.courseId) === parsedCourseId)
      ) {
        toast.error(tr("Tạo lớp học thất bại"));
        return;
      }
    ...
    .map((course) => {
      ...
      const matchedClasses = course.classes.filter((cls) => {
        ...
        if (statusFilter === "ACTIVE")
          return (
            matchesClassSearch &&
            (clsSt.includes("diễn ra") || clsSt.includes("active"))
          );
        if (statusFilter === "UPCOMING")
          return (
            matchesClassSearch &&
            (clsSt.includes("sắp") || clsSt.includes("upcoming"))
          );
        if (statusFilter === "COMPLETED")
          return (
            matchesClassSearch &&
            (clsSt.includes("kết thúc") || clsSt.includes("completed"))
          );
    ...
    <div className="course-class-management-page">
      ...
      <section className="content-header">
        ...
        >
          ...
          >
            ...
            >
              <path
                d="M6.66667 11.6667H8.33333V8.33333H11.6667V6.66667H8.33333V3.33333H6.66667V6.66667H3.33333V8.33333H6.66667V11.6667ZM1.66667 15C1.20833 15 0.815972 14.8368 0.489583 14.5104C0.163194 14.184 0 13.7917 0 13.3333V1.66667C0 1.20833 0.163194 0.815972 0.489583 0.489583C0.815972 0.163194 1.20833 0 1.66667 0H13.3333C13.7917 0 14.184 0.163194 14.5104 0.489583C14.8368 0.815972 15 1.20833 15 1.66667V13.3333C15 13.7917 14.8368 14.184 14.5104 14.5104C14.184 14.8368 13.7917 15 13.3333 15H1.66667ZM1.66667 13.3333H13.3333V1.66667H1.66667V13.3333Z"
                fill="currentColor"
              />
            </svg>
      ...
      <section className="courses-dashboard-top">
        <div className="stats-card-dark">
          <div className="card-bg-icon">
            ...
            >
              <path
                d="M26.6667 93.3333H40V60H26.6667V93.3333ZM80 93.3333H93.3333V26.6667H80V93.3333ZM53.3333 93.3333H66.6667V73.3333H53.3333V93.3333ZM53.3333 60H66.6667V46.6667H53.3333V60ZM13.3333 120C9.66667 120 6.52778 118.694 3.91667 116.083C1.30556 113.472 0 110.333 0 106.667V13.3333C0 9.66667 1.30556 6.52778 3.91667 3.91667C6.52778 1.30556 9.66667 0 13.3333 0H106.667C110.333 0 113.472 1.30556 116.083 3.91667C118.694 6.52778 120 9.66667 120 13.3333V106.667C120 110.333 118.694 113.472 116.083 116.083C113.472 118.694 110.333 120 106.667 120H13.3333ZM13.3333 106.667H106.667V13.3333H13.3333V106.667ZM13.3333 13.3333V106.667V13.3333Z"
                fill="white"
              />
            </svg>
          </div>
            ...
            <div className="stats-row">
              <div className="stat-box">
                ...
                <p className="stat-value">
                  {String(allCoursesRaw.length).padStart(2, "0")}
                </p>
              </div>
              <div className="stat-box">
              ...
              <div className="stat-box">
                ...
                <p className="stat-value">
                  {String(allSubjects.length).padStart(2, "0")}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="table-toolbar">
            <div className="toolbar-left">
              <div className="search-box">
                ...
                >
                  <path
                    d="M11.5 6.5C11.5 9.26142 9.26142 11.5 6.5 11.5C3.73858 11.5 1.5 9.26142 1.5 6.5C1.5 3.73858 3.73858 1.5 6.5 1.5C9.26142 1.5 11.5 3.73858 11.5 6.5ZM16 14.5L11.5 10"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                  />
                      ...
                      >
                        ...
                        <div className="col-structure">
                          <div className="structure-badges-row">
                            {Object.entries(course.structure).map(
                              ([key, val]) => (
      ...
      {!loading && orphanClasses.length > 0 && (
        <section
          ...
          style={{ marginTop: "24px", border: "1px solid #fecaca" }}
        >
          <div
          ...
          >
            ...
            >
              <div className="panel-header">
                <h2>
                  {tr("Xuất báo cáo lớp")} —{" "}
                  {exportClassTarget.name || exportClassTarget.code}
                </h2>
                <div
              ...
              >
                ...
                {EXPORT_TYPES.map((t) => (
                  <button
                    ...
                    onClick={() =>
                      handleExportReport(exportClassTarget.classId, t.type)
                    }
                    style={{
                      padding: "12px 16px",
                      borderRadius: "8px",
                      ...
                      background: "#f8fafc",
                      cursor:
                        exportingType !== null ? "not-allowed" : "pointer",
                      fontSize: "13px",
                      fontWeight: "700",
                      color: "#002147",
                      display: "flex",
                  ...
                  >
                    <span>📄 {t.label}</span>
                    {exportingType === t.type && (
                      <span>{tr("Đang xuất...")}</span>
                    )}
                  </button>
                ))}
                {exportError && (
