...
const EditLearner = ({ learner, onSave, onCancel }) => {
  ...
  useEffect(() => {
    const loadAssignments = async () => {
      try {
        ...
        // Find enrollments for this learner
        const learnerEnrollments = enrArr.filter(
          (e) => e.accountId === learner.accountId
        );

        // Merge enrollment + class data
        const assigned = learnerEnrollments.map((enr) => {
          const classInfo = clsArr.find((c) => c.classId === enr.classId);
          return {
            enrollmentId: enr.enrollmentId,
            classId: enr.classId,
            code: classInfo?.classCode || `CLS-${enr.classId}`,
            name: classInfo?.className || `Lớp #${enr.classId}`,
  ...
  const handleDeleteClass = async (classCode) => {
    const cls = assignedClasses.find((c) => c.code === classCode);
    if (!cls || !cls.enrollmentId) return;
  ...
  const handleAddClass = async () => {
    ...
    try {
      ...
      // Add to local state
      const newAssignment = {
        enrollmentId: result?.enrollmentId || Date.now(),
        classId: classToAdd.classId,
        code: classToAdd.classCode,
